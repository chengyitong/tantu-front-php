		...
		if($v) {
			...
			if($arr['after_today_gray'] && $curdate > $eachdate){
				...
				foreach($datas[$eachdate] as $k){
					...
					<a class="sexybox_iframe" href="/index/diary_v?id='.$k['id'].'" style="color:blue" hidefocus="true">'.$k['title'].'</a>
					<div style="position: absolute; right: 0px; top: 0px; display: none; background: rgb(255, 255, 255);" class="do'.$k['id'].'">
						<a href="/index/diary_del" data-id="'.$k['id'].'" class="btn_del" callback="" style="color:blue;" hidefocus="true">×</a>
					</div>
					</li>';
				}
				$html .= '</ul>
					<div id="adds'.$eachdate.'" style="position:absolute;top:3px;right:5px;font-size:12px;display:none">
					<a class="sexybox_iframe" href="/index/diary_addv?d='.$dates.'" style="color:blue">添加</a>
					</div>
					</td>';
			}
		} else {
