...
var Global = function(){
	...
	this.bind_btn = function(){
		...
		$('.btn_open').click(function(){
			...
			global.openNew(url,title,cback);
		});
		//  绑定删除按钮
	...
	// div首部跟随
	this.FixDiv = function(TableID) {
		var width=$('#'+TableID).width();
		//  创建fix div
		$("#" + TableID).after("<div id='" + TableID + "_tableHead' style='display:none;overflow:hidden;position:fixed;top:0;z-index:45;width:"+width+"'></div>");
		//  复制table
		var oldtable = $("#" + TableID);
		var tableHeadClone = oldtable.clone(true);
		//  定义新table id
	...
	//  获取url参数
	this.iget = function(name){
		 var reg = new RegExp("(^|&)"+ name +"=([^&]*)(&|$)");
		 var r = window.location.search.substr(1).match(reg);
		 if(r!=null)return  unescape(r[2]); return null;
	}
	//  关闭弹出窗口
	this.ExitWin = function(str){
		//window.returnValue="";
		if(!str) str = '';
		func = global.iget('func');
		if(func){
			try{
				eval( 'window.opener.'+func );
			}catch(e){}
		}else window.opener.global.rvopenwin(str);
		window.close();
	}
	//  关闭上层框架弹出窗口
	this.ExitPWin = function(str){
		//parent.window.returnValue="";
		if(!str) str = '';
		func = parent.global.iget('func');
		if(func){
			try{
				eval( 'parent.window.opener.'+func );
			}catch(e){}
		}else parent.window.opener.global.rvopenwin(str);
		parent.window.close();
	}
	//  mis打开弹窗
	this.openNew = function(url,title,func){
		title = title ? title : '业务窗口';
		if(!func) func = '';
		var iWidth=window.screen.availWidth; //弹出窗口的宽度;
		var iHeight=window.screen.availHeight; //弹出窗口的高度;
		var iTop = (iHeight/2)-290; //获得窗口的垂直位置;
		var iLeft = (iWidth/2)-440; //获得窗口的水平位置;
		window.open('/index/iu?url='+this.urlEncode(url)+"&title="+this.urlEncode(title)+"&func="+func+"&rad="+Math.random(),'','width=880,height=580,resizable=no,scrollbars=no,status=no,left='+iLeft+',top='+iTop);
	}
	this.rvopenwin = function(res){
		if (res=="refrash")
			location.reload();
...
var Loading = function(){
	//  预缓存图片
	this.init = function(){
		$('body').append('<div style="width:0;height:0;position:absolute;right:0;bottom:0;"><span style="background:url(/static/img/information.png);" /><span style="background:url(/static/img/note.gif);" /><span style="background:url(/static/img/loading2.gif);" /></div>');
	}
	//  显示
	this.show = function(who,str,img,cls){
		if(!who) who=0;
		if(!str) str='数据加载中，请稍等...';
		if(cls==1) str+='<div id="loadingclose" onclick="loading.hide();"></div>';
		if(loading_state){
			if(who==0) $('#progressBar').html(str);
			else parent.$('#progressBar').html(str);
			loading.size();
		}else{
			$('body').append('<div id="background"></div><div id="progressBar">'+str+'</div>');
			loading.size();
			$(window).resize(function(){
				loading.size();
			});
	...
	this.size = function(){
		...
		$('#progressBar').css('left',$(window).outerWidth()/2-($('#progressBar').outerWidth()/2));
	}
	//  更换图片
	this.cimg = function(sel){
		if(sel==2) str = 'information.png';
		else if(sel==3) str = 'note.gif';
		else str = 'loading2.gif';
		$('#progressBar').css('backgroundImage','url(/static/img/'+str+')');
	}
}
var Jdate = function(){
	//+---------------------------------------------------
