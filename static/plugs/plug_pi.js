...
function ScrollPic(scrollContId,arrLeftId,arrRightId,dotListId,listType){
	...
	this.dotOnClassName   = "selected";//当前点className
	this.dotObjArr = [];
	this.listEvent = "onclick";
	this.circularly = true; //循环滚动（无缝循环）
	this.pageWidth = $(document).width(); //翻页宽度
	this.frameWidth = $(document).width(); //显示框宽度
	this.speed = 10; //移动速度(单位毫秒，越小越快)
	this.space = 30; //每次移动像素(单位px，越大越快)
...
function call_scrollpic(){
	$('.AutoPlay').each(function(){
		var autoplay = $(this).data('autoplay');
		var hasload = $(this).data('hasload');
		var play = $(this).data('play');
		if(play==undefined) play=true;
		if(autoplay!=undefined && hasload==undefined){
			$(this).data('hasload','play');
			var slide_id = 'slide_'+autoplay;
			var point_id = 'point_'+autoplay;
			$(this).find('.slide').attr('id',slide_id);
			$(this).find('.Point').attr('id',point_id);
			var slide_run = new ScrollPic();
			slide_run.scrollContId   = slide_id; //内容容器ID
			slide_run.dotListId      = point_id;//点列表ID
			slide_run.pageWidth = $(this).data('width');
			slide_run.frameWidth = $(this).data('width');
			slide_run.autoPlay = play;
			slide_run.initialize(); //初始化
		}		
	});
}
$(function(){
...
ScrollPic.prototype = {
	...
	initialize : function(){ //初始化
		...
		this.stripDiv.style[this.upright?'height':'width'] = "9999999px";
		if(!this.upright){	
	...
	_touchmove : function(e){
		...
		if(this.iPadStatus == 'ok'){
			if(this.iPadScrollY == window.pageYOffset && this.iPadScrollX == window.pageXOffset && Math.abs(cX)>20){ //横向触摸
