...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        url = res.url;
        //str = "<div><strong>Link:</strong><a href=" + res.url + " target='_blank' > " + res.url + "</a></div><div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        //str = "<div><strong>Link:</strong><a href=" + url + " target='_blank' > " + link + "</a></div><div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }
	str = 'Successful!';
...
return;
    ...
    var imageView = '-slist?imageView2/1/w/100/h/100';
    ...
    var showImg = $('<img src="/static/qiniu/jssdk/demo/images/loading.gif"/>');

    progressNameTd.append(Wrapper);

    if (!isImg) {
        showImg.attr('src', '/static/qiniu/jssdk/demo/images/default.png');
        ...
        $(img).on('load', function() {
            ...
            linkWrapper.attr('href', purl).attr('title', '查看原图');
            ...
            function initImg(url, key, height) {
                ...
                modalBody.find('img').attr('src', '/static/qiniu/jssdk/demo/images/loading.gif');
