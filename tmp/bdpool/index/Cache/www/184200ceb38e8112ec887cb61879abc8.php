        ...
        <div class="fans">
            <div class="fans-list clearfix">
                <div class="fans-margin">
                    <?php if(is_array($data["folder"])): $i = 0; $__LIST__ = $data["folder"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vof): $mod = ($i % 2 );++$i;?><a href="/camerist/album_list?uid=<?php echo ($target_user["id"]); ?>&id=<?php echo ($vof["id"]); ?>">
                        <div class="list2 float" style="border:1px solid #cdcdcd;<?php if($vof['img_count']==0) echo 'display:none;'; ?>">
                        <div style=" width:100%; height:272px;background:url(<?php echo ($vof["topimg"]); ?>);background-position:center;background-size:cover;border-bottom:1px solid #cdcdcd;">
                        </div>
                            <div style="font-size:20px; text-align:center; margin-top:20px"><?php echo ($vof["foldername"]); ?></div>
                            <div style=" text-align:center; margin:10px; color:#b9c1c7"><?php echo ($vof["img_count"]); ?>张图片</div>
                        </div>
