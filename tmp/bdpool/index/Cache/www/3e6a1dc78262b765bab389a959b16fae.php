...
<div class="container">
    <div class="seacon1b clearfix">
            <div class="input-group seacon1input">
                <input type="text" onKeyDown="if(window.event.keyCode==13) {searchCamerlist();}" class="form-control searchTxt" id="searchcon" value="<?php echo unescape(I('get.kw'));?>" placeholder="摄影师昵称">
...
<!--修改的欢迎摄影师-->
<!-- <div class="fanye" style="text-align: center;"><?php echo ($data["fanye"]); ?></div>      -->
<div class="container searchcameristcontainer">
        <div class="con2 row">
            <?php if(is_array($data["user_list"])): $i = 0; $__LIST__ = $data["user_list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vouser): $mod = ($i % 2 );++$i;?><div class="searchcameristcons3 col-lg-3 col-md-3 col-sm-4 col-xs-12">
                    <div class="con3">
                        <a href="/camerist/gallery?uid=<?php echo ($vouser["id"]); ?>" target="_blank">
                            <div class="con4" style="background:url(<?php echo camerist_t($vouser['albumimg']);?>) #FFF;background-position: center;background-size:cover;"></div>
                        </a>
                        <div class="con5"><a href="/camerist/gallery?uid=<?php echo ($vouser["id"]); ?>" target="_blank"><img class="img-circle lazy" data-original="/static/<?php if($vouser['avatar']) echo 'uploadfiles/images/'.$vouser['avatar'];else echo 'images/tx.png'; ?>" src="" style="width: 60px; height: 60px;background:#fff;"></a>
                            ...
                            <?php if($vouser['id']!=$data['user']['id']){ ?><span onClick="fouc(<?php echo ($vouser["id"]); ?>,this)" class="<?php if($vouser['followed']==1) echo 'fansbtn_2'; else echo 'fansbtn'; ?>"><?php if($vouser['followed']==1) echo '已关注';else echo '关注'; ?></span>
                            <?php } ?>
                        </div>
                    </div>
...
<div class="fanye" style="text-align: center;padding-right:0px;margin-bottom: 20px;"><?php echo ($data["fanye"]); ?></div>
