    ...
    <table width="100%" border="1" cellpadding="5" cellpadding="1">
        <tr>
            ...
            <td><span class="title">性别：</span>
                <?php if($target_user['gander']==1) echo '男';else echo '女'; ?>
            </td>
            <td><span class="title">邮箱：</span>
                <?php if($target_user['email_show']) echo $target_user['email']; else echo '保密'; ?>
            </td>
            <td><span class="title">手机：</span>
                <?php if($target_user['mobile_show']) echo $target_user['mobile']; else echo '保密'; ?>
            </td>
        </tr>
        <tr>
            <td><span class="title">生日：</span>
                <?php if($target_user['birth']!='0000-00-00') echo $target_user['birth']; ?>
            </td>
            <td colspan="3"><span class="title">现居：</span><?php echo str_replace('|;|',' ',$target_user['address']);?></td>
        </tr>
        <tr>
            <td style="color: #09f;"><span class="title">身份：</span>
                <?php $sf = D('syscode')->where('syscode_id in (0'.$target_user['shenfen'].'0) and syscode_parentid=318 and syscode_isuse=1')->order('syscode_sortno')->select(); ?>
                <?php if(is_array($sf)): $i = 0; $__LIST__ = $sf;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; echo ($vo["syscode_name"]); ?>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
            </td>
            <td colspan="3"><span class="title">简介：</span><?php echo ($target_user["introduce"]); ?></td>
        </tr>
    </table>
</div>
