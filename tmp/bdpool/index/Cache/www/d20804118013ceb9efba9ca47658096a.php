...
<div class="container1200 center padding_b">  
  ...
  <div class="pcon42 left">
    ...
    <?php if(is_array($data["folder"])): $i = 0; $__LIST__ = $data["folder"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vof): $mod = ($i % 2 );++$i;?><div class="album1 ff<?php echo ($vof["id"]); ?>">
	    <a href="/user/album_list?id=<?php echo ($vof["id"]); ?>">
	    <div class="album11" style="background:url(<?php echo ($vof["topimg"]); ?>);background-position: center;background-size:cover;">
	    </div>
	    </a>
	    <div class="album12">
		    <a href="/user/album_list?id=<?php echo ($vof["id"]); ?>"><h4 style="padding-top:20px;"><?php echo ($vof["foldername"]); ?><span style="padding-left:10px; color:#9fa0a0"><?php echo ($vof["img_count"]); ?>张图片</span></h4></a>
            <?php if($vof['def']!=1){ ?>
		    <span class="discuz" style="position:absolute; right:5px; top:20px;cursor: pointer;"><img data-toggle="modal" data-fname="<?php echo ($vof["foldername"]); ?>" data-id="<?php echo ($vof["id"]); ?>" data-target="#album" class="colpencil" src="/static/images/colpencil2.png" width="20" height="18" />&nbsp;<img onclick="delfolder(<?php echo ($vof["id"]); ?>)" src="/static/images/rbsb.png" width="20" height="18" /></span>
            <?php } ?>
	    </div>
