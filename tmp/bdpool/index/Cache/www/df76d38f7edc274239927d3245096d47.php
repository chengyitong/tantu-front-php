  ...
  <div class="fans">
    <div class="fans-list clearfix" style="margin-top:20px;">
      <div class="fans-margin">
      	<?php if(!$data['list']) echo '暂没收藏图片'; ?>
      	<?php if(is_array($data["list"])): $i = 0; $__LIST__ = $data["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="list3 float ff<?php echo ($vo["id"]); ?>">
          <div class="list3 float">
            <div class="list32 float">
              <div onclick="location='/user/collection_list?id=<?php echo ($vo["id"]); ?>';" style="cursor:pointer;width:100%; height:272px;background:url(<?php echo ($vo["topimg"]); ?>);background-position: center;background-size:cover;border-bottom:1px solid #eeeff2;" ></div>
              <div style="font-size:20px; text-align:center; margin-top:20px"><?php echo ($vo["foldername"]); ?>  <img src="/static/images/<?php if($vo['show']) echo 'unlock.png'; else echo 'locked.png'; ?>" height="20" /></div>
              <div style=" text-align:center; margin:10px; color:#b9c1c7"><?php echo ($vo["img_count"]); ?>张图片</div>
              <div class="discuz" style="position:absolute; right:10px; top:10px" onclick="delfolder(<?php echo ($vo["id"]); ?>)"><img src="/static/images/colclose.png" /></div>
              <div class="discuz" style="position:absolute;bottom: 80px;padding: 5px 10px; height:37px;width: 100%;background: rgba(0, 0, 0, 0.2) none repeat scroll 0% 0%; ">
                 <div style="float:right;margin-top:4px;" data-toggle="modal" data-fname="<?php echo ($vo["foldername"]); ?>" data-show="<?php echo ($vo["show"]); ?>" data-id="<?php echo ($vo["id"]); ?>" data-target="#myalbum" class="colpencil">
